...
void *gaus(void* args) { 
  ...
  for(k = 0; k < matrix_size; k++){
    ...
    for(j = (k + 1 + start); j < end; j++){
        ...
        U.elements[matrix_size * k + j] = (float)(U.elements[matrix_size * k + j] / U.elements[matrix_size * k + k]);	/* Division step */
        ...
        for (i = (k + 1 + start); i < end; i++) {
            //printf("f\n");
            for (j = (k + 1 + start); j < end; j++){
                pthread_mutex_lock(lock);
                //printf("g\n");
                U.elements[matrix_size * i + j] = U.elements[matrix_size * i + j] - (U.elements[matrix_size * i + k] * U.elements[matrix_size * k + j]);	/* Elimination step */
                pthread_mutex_unlock(lock);
            }
            pthread_mutex_lock(lock);
            //printf("h\n");
