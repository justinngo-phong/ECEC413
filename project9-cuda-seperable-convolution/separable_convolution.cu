                   ...
                   int num_rows, int half_width)
{
	...
	cudaMalloc((void**)&kernel_ond, kernel_size * sizeof(float));
	cudaMemcpy(kernel_ond, kernel, kernel_size * sizeof(float), cudaMemcpyHostToDevice);
	...
	/*Execute naive implementation on GPU */

	convolve_rows_kernel_naive<<<grid, thread_block>>>(rowtocol, matrix_ond, kernel_ond, num_rows, num_cols, half_width);

	cudaDeviceSynchronize();

	check_for_error("KERNEL FAILURE");

	convolve_columns_kernel_naive<<<grid, thread_block>>>(gpu_result_ond, rowtocol, kernel_ond, num_rows, num_cols, half_width);
